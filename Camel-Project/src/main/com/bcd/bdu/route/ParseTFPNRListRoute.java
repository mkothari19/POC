...
public class ParseTFPNRListRoute extends RouteBuilder {
	public void configure(){
    	...
    	.process(new Processor(){
    		public void process(Exchange exchange) throws Exception {
    			...
    			exchange.getIn().setBody("${date:now:yyyyMMddHHmmss}: ParseTFPNRListRoute : " + filename+"\r\n");
    		}
    	})    	
		...
		from("direct:parse_tf_pnr_list")
		.routeId("TFParsePNRLISTRoute")
		.process(new Processor() {
			@Override
			public void process(Exchange exchange) throws Exception {
		...
		.to("file://[(api.endpoint.tf.pnr.parse.ids)]")
