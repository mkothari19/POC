...
object KafkaIntigration {
 ...
 val kafkaParam=Map[String,Object](ConsumerConfig.BOOTSTRAP_SERVERS_CONFIG -> "localhost:9092",
  ...
  ConsumerConfig.AUTO_OFFSET_RESET_CONFIG-> "earliest",
   ConsumerConfig.ENABLE_AUTO_COMMIT_CONFIG  -> "false", "spark.kafka.poll.time" -> "5000", "spark.streaming.kafka.maxRatePerPartition" -> "25", "spark.streaming.unpersist" -> "true"
 ...
 //LocationStrategies.PreferFixed
  ...
  //val s=stream.map(record => (record.value))
     ...
     if(!rdd.isEmpty()){
       ...
       df.printSchema()
     // spark.sqlContext.sql("SELECT BookingDateTime from PNR").show()
